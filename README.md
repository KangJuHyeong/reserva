...
`k6`와 대량 시드 데이터를 이용해 `1000 / 5000 / 10000` 이벤트 규모에서 공개 이벤트 탐색 API를 반복 측정했고, 특히 `10000건` 구간에서 홈 화면 공개 이벤트 목록 조회 성능이 급격히 악화되는 지점을 확인했습니다.

이후 `GET /api/v1/events`를 담당하는 QueryDSL 코드를 직접 열어 흐름을 확인했고, 다음 순서로 병목을 줄였습니다.
- 1차: 페이지네이션의 `total`을 구하는 `countQuery`가 항상 무거운 join을 타고 있어, 검색이나 섹션 조건이 실제로 필요할 때만 join하도록 단순화
- 2차: 목록 조회를 한 번에 fetch join으로 끝내지 않고, 먼저 페이지 대상 `id`를 구한 뒤 그 결과에 대해서만 `creator`, `inventory`를 fetch join하도록 분리
- 3차: 검색 predicate에서 비용이 큰 `description` 전체 검색을 제거하고 `title / location / creator` 중심으로 좁힘

즉 이번 성능 최적화는 막연히 인덱스를 추가한 작업이 아니라, "측정으로 병목 확인 -> 공개 탐색 쿼리 코드 확인 -> count 단순화 -> 조회 구조 분리 -> 검색 범위 축소" 순서로 진행한 작업이었습니다.
