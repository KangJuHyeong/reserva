...
export function ReservationCard({ reservation, onToggleWatchlist }: ReservationCardProps) {
      ...
      <Link href={`/reservation/${reservation.id}`}>
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <div className="absolute left-3 top-3 flex gap-2">
            <span className={cn("rounded-full px-2.5 py-1 text-xs font-medium", categoryColors[reservation.category])}>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <Button
            ...
            className={cn(
              ...
              reservation.isWatchlisted && "text-destructive"
            )}
            onClick={(e) => {
              e.preventDefault();
              onToggleWatchlist(reservation.id);
            }}
          >
            <Heart className={cn("h-4 w-4", reservation.isWatchlisted && "fill-current")} />
          </Button>
        </div>
      </div>
    </div>
